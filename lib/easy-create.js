...
const { pauseSpinner } = require('../lib/util/spinner.js');
const Creator = require('./Creator');
const { clearConsole } = require('./util/clearConsole.js');

const TEMPLATE_NAME_WHITE_LIST = ['node', 'vue'];

async function create(templateName, projectName, options) {
  // TODO options方便后续扩展
  // 项目模板白名单
  if (!TEMPLATE_NAME_WHITE_LIST.includes(templateName)) {
  ...
  // console.log('path', targetDir, name, validateResult); // logger code
